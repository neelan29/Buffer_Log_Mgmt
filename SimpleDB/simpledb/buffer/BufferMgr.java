...
public class BufferMgr {
   ...
   public synchronized Buffer pinNew(String filename, PageFormatter fmtr) {
      try {
         long timestamp = System.currentTimeMillis();
         Buffer buff = basicBufferMgr.pinNew(filename, fmtr);
         while (buff == null && !waitingTooLong(timestamp)) {
            wait(MAX_TIME);
            buff = basicBufferMgr.pinNew(filename, fmtr);
         }
         if (buff == null)
            throw new BufferAbortException();
         return buff;
   ...
   */  
   public boolean containsMapping(Block blk) {  
	   return basicBufferMgr.containsMapping(blk);  
   } 
   /**  
   ...
   */  
   public Buffer getMapping(Block blk) {  
	   return basicBufferMgr.getMapping(blk);  
   }  
   
   public ArrayList<Buffer> getBufferHistory() {  
	   return basicBufferMgr.getBufferHistory();  
   } 
   
   public ArrayList<Block> getBlockHistory() {  
	   return basicBufferMgr.getBlockHistory();  
   } 
